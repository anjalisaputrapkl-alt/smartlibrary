// Members Stats Modal Manager
const membersStatsModal = {
    init() {
        console.log('membersStatsModal.init() called');
    ...
    setupCardListeners() {
        const cards = document.querySelectorAll('.stat-card[data-stat-type]');
        cards.forEach(card => {
    ...
    openModal(type) {
        ...
        const titles = {
            'total': 'Daftar Semua Murid',
            'email': 'Murid dengan Email Terdaftar'
        };
        
        overlay.querySelector('.modal-header h2').textContent = titles[type] || 'Detail Murid';
    ...
    async fetchAndDisplayData(type) {
        const endpoints = {
            ...
            'email': '/perpustakaan-online/public/api/get-members-email.php'
        };
        ...
        html += `
            <thead>
                <tr>
                    ...
                    <th class="col-hide-mobile">Tgl Gabung</th>
                </tr>
            </thead>
            <tbody>
        `;
        ...
        data.forEach(member => {
            html += `
                <tr>
                    ...
                    <td class="col-hide-mobile" style="font-size: 12px;">${member.joined_date}</td>
                </tr>
            `;
        });
