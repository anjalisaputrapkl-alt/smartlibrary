...
try {
    ...
    try {
        $stmt = $pdo->prepare("
            ...
            FROM members m
            WHERE m.school_id = :sid2 AND m.email IS NOT NULL AND m.email != ''
        ...
        error_log('get-members-email.php Error: ' . $e->getMessage());
    ...
    error_log('get-members-email.php Fatal Error: ' . $e->getMessage() . ' in ' . $e->getFile() . ':' . $e->getLine());
    http_response_code(500);
    echo json_encode([
        'success' => false,
