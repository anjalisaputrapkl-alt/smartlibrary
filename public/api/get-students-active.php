...
try {
    ...
    try {
        $stmt = $pdo->prepare("
            ...
            FROM members m
            WHERE m.school_id = :sid2 AND m.status = 'active'
            ORDER BY m.name ASC
        ");
        ...
        error_log('get-students-active.php Error: ' . $e->getMessage());
    ...
    error_log('get-students-active.php Fatal Error: ' . $e->getMessage() . ' in ' . $e->getFile() . ':' . $e->getLine());
    http_response_code(500);
    echo json_encode([
        'success' => false,
